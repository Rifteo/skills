...
def main():
    ...
    parser.add_argument("--params", nargs="+", default=["/search?q=FUZZ", "/page?name=FUZZ"],
                        help="Parameterized paths with FUZZ marker")
    parser.add_argument("--submit-endpoint", help="Endpoint to submit stored XSS payload")
    parser.add_argument("--display-endpoint", help="Endpoint where stored input is rendered")
    parser.add_argument("--field", default="body", help="JSON field name for stored XSS (default: body)")
    parser.add_argument("-o", "--output", default="xss_report.json")
    args = parser.parse_args()

    # Support cookie auth in addition to Bearer token
    if args.cookie and not args.token:
        import functools, types
        _orig = requests.get
        def _get_with_cookie(url, **kw):
            kw.setdefault("headers", {})["Cookie"] = args.cookie
            return _orig(url, **kw)
        requests.get = _get_with_cookie
    ...
    if args.submit_endpoint and args.display_endpoint:
        findings.extend(test_stored_xss(args.base_url, args.submit_endpoint,
                                         args.display_endpoint, args.token or "",
                                         field=args.field))
