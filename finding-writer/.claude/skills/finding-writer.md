...
3. **Assess Severity**: Score based on actual exploitability and impact in context, not just the vulnerability class — see `~/agent-skills/finding-writer/references/severity-guide.md`. Run `~/agent-skills/finding-writer/scripts/cvss-scorer.py` to produce the CVSS v3.1 vector and base score.
4. **Tag the Finding**: Run `~/agent-skills/finding-writer/scripts/cwe-search.py <keyword>` to look up the correct CWE ID and OWASP category for the vulnerability type.
...
**References:** CWE ID, OWASP Top 10 category, and/or CVE if applicable. See `~/agent-skills/finding-writer/references/owasp-cwe-map.md` for common mappings.
